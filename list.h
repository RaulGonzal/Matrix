...
{
    public:
    Node(T v, int cn);				    // parameterized constructor
    T getVal() const;		    // getter for Value
    int getColumn_num() const;    // getter for Column number
    Node<T>* getNext() const;	// getter for the next pointer
    void setVal(T v);		    // setter for value
    void setColumn_num(int cn); //setter for column number
    ...
    int column_num;         // The coloumn number the value is located
};
...
class List
{
    ...
    Node<T>* getHead() const;       //getter for head pointer 
    ...
    void addElement(T, int);			// Adds an element at the end of the list
    bool insert(T, int, int);    		// inserts an element at a given position
    bool remove(T, int);					// removes an element from the list
    bool search(T, int) const;			// looks for the given item in the list
