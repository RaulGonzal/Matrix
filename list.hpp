...
template <class T>
int Node<T>::getColumn_num() const
{
    return column_num;
}
...
template <class T>
void Node<T>::setColumn_num(int cn)
{
    column_num = cn;
}
 ...
 */
template <class T>
void List<T>::addElement(T v, int cn)
{
    Node<T> *cur,*incoming;
	// create a new Node
    incoming = new Node<T>(v,cn);
...
void List<T>::print() const
{
    ...
    while(cur!=NULL)
    {
        cout<<cur->getColumn_num() << "," << cur->getVal()<<"\t";
        cur = cur->getNext();
    }
    cout<<endl;
}
 ...
 */
template<class T>
bool List<T>::insert(T val, int cn, int pos)
{
    if(pos <0 || pos>size())	// fails
        return false;
    
	// create a new node
    Node<T> *newNode = new Node<T>(val, cn);
...
bool List<T>::remove(T val, int cn)
{
    ...
    while(cur!=NULL && !done)
    {
        if(cur->getVal() == val && cur->getColumn_num() == cn)
        {
 ...
 */
template <class T>
bool List<T>::search(T val, int cn) const
{
    Node<T> *cur = head;
    bool found = false;
...
List<T>::List(const List<T> &oldList)
{
    ...
    while(el!=NULL)
    {
        addElement(el->getVal(), el->getColumn_num());
        el = el->getNext();
    }
}
...
{  
    l.clear();            
    string str;                                                                                // the entire line of input 
    T val;
    int i = 0;                                                                                 // column number
    T null_val;                                                                                // the first input char, will be null value

    getline(cin, str);                                                                         // gets the entire row
    stringstream ss(str);                                                                      // splices the row
    ss >> null_val;                                                                            // sets null value
    while (ss >> val)                                                                          // reads in all values after null
    {
        if (val != null_val)                                                                   
        {
            l.addElement(val, i);                                                              // if not null, add input to list
        }
        i++;                                                                                   // next column
    }
...
// Adittion operator overload.
template <class T>
List<T> List<T>::operator+(const List<T> &l1)
{                                       

    Node<T> *curr = head;                                                                       // points to head of the first list
    Node<T> *el = l1.head;                                                                      // points to head of the second list
    List<T> newList;                                                                            // will hold the sum of the two lists
    
    while(el!=NULL && curr != NULL)                                                             // while both list are not at the end / dereferenced
    {
            if(curr->getColumn_num() == el->getColumn_num())                                    // if the current nodes have matching colums
            {
                newList.addElement((curr->getVal() + el->getVal()), curr->getColumn_num());     // adds the values of both nodes into new list
                curr = curr->getNext();                                                         // move to next node of list 1
                el = el->getNext();                                                             // move to next node of list 2
            }
            else if(curr->getColumn_num() < el->getColumn_num()){                               
                newList.addElement(curr->getVal(), curr->getColumn_num());                      // add the node with smaller col to new list
                curr = curr->getNext();                                                         // iterate through the list 1
            }
            else if(el->getColumn_num() < curr->getColumn_num()){                                                      
                newList.addElement(el->getVal(), el->getColumn_num());                          // add the node with the smaller col to new list
                el = el->getNext();                                                             // iterate through the list 2
            }
        }
        while(curr != NULL){                                                                    // when reached the end of list 2 but not the first
            newList.addElement(curr->getVal(), curr->getColumn_num());                          // add all of the remaining nodes to new list
            curr = curr->getNext();
        }
        while(el != NULL){                                                                      // when reached the end of the list 1 but not the 2nd
            newList.addElement(el->getVal(), el->getColumn_num());                              // add all of the remaining nodes in the list 2
            el = el->getNext();
        }
